...
def parse_variable_groups(variables,variable_groups):
    ...
    for gp_name,gp_variables in variable_groups.items():
            variable_group_dict[gp_name] = [np.array([variables.index(x) for x in lst]) for lst in gp_variables]
...
class VariableGroup:
    ...
    def __call__(self,x) -> Any:
        ...
        for gp_name,gp_variables in self.variable_group_dict.items():
            out_dict[gp_name] = torch.stack([x[...,lst,:,:] for lst in gp_variables],-4)
