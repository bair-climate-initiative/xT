...
def build_attention_mask_adjacent(input_resolution,radius,grid=None):
    ...
    abs_coords_diff = torch.abs( abs_coords[:,None] - abs_coords[None,:]).sum(-1) # N N 4
    ...
    mask = torch.where(mask, torch.tensor(0.0, dtype=torch.float32), torch.tensor( float("-inf"), dtype=torch.float32)) # 1 if in, 0 if out
    return mask
