...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,attention_mode='default',temperature=1.0,
                 rel_pos_bias=False,input_resolution=None,radius=1,grid=None):
        super().__init__()
        ...
        if self.attention_mode == 'cosine':
            ...
            self.radius = radius
            atten_bias = build_attention_mask_adjacent(self.input_resolution,radius=self.radius,grid=self.grid)
            mask = atten_bias==0
            edges = torch.where(mask)
            self.register_buffer("edges", torch.stack(edges), persistent=False)
            self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((1, 1, num_heads))))
        ...
        if self.rel_pos_bias:
            ...
            abs_coords = [torch.arange(x, dtype=torch.float32) for x in self.input_resolution]
            abs_coords =  torch.stack(torch.meshgrid(abs_coords),dim=-1).contiguous()  # T L H W 4
            ...
            if self.attention_mode == 'xformer':
                ...
                abs_coords_x = abs_coords_x + abs_coords_y * 0.
                abs_coords_y = abs_coords_y + abs_coords_x * 0.
                abs_table = None
                if self.rel_pos_bias == 'abs':
                    abs_table = torch.cat(
    ...
    def forward(self, x,mask=None):
        ...
        if self.attention_mode == 'default':
            ...
            edges = self.edges
            edge_features = torch.cat([self.abs_coords[edges[0]],self.abs_coords[edges[1]]],dim=-1)
            ...
            qk = qk.exp()
            q_exp_sum = torch.zeros(q.shape[0],q.shape[1],q.shape[2],dtype=q.dtype,device=q.device)
            q_exp_sum = q_exp_sum.scatter_add(dim=1,index=edges[0][None,:,None].expand(q.shape[0],-1,q.shape[2]),src=qk)
            ...
            out = out.scatter_add(dim=1,index=edges[0][None,:,None,None].expand(q.shape[0],-1,q.shape[2],q.shape[3]),src=res)
...
class ViTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 rel_pos_bias=False,num_patches=None,norm_mode='post',**kwargs):
        super().__init__()
        self.norm1 = norm_layer(dim)
        input_resolution = num_patches 
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,input_resolution=input_resolution,rel_pos_bias=rel_pos_bias)
...
class GlobalLocalAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm4d,rel_pos_bias=False,num_patches=None,norm_mode='post',**kwargs):
        super().__init__()
        ...
        self.attn_spatial = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,attention_mode='cosine',rel_pos_bias=rel_pos_bias,input_resolution=(input_resolution[2],input_resolution[3]))
        self.attn_local = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,attention_mode='cosine',rel_pos_bias=rel_pos_bias,input_resolution=(input_resolution[0],input_resolution[1]))
...
class GraphAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm4d,rel_pos_bias=False,num_patches=None,norm_mode='pre',radius=1,grid=None,**kwargs):
        super().__init__()
        ...
        input_resolution = num_patches 
        self.attn_ = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,attention_mode='xformer',rel_pos_bias=rel_pos_bias,input_resolution=(input_resolution[0],input_resolution[1],input_resolution[2],input_resolution[3]),radius=radius,grid=grid)
...
class ThreeWayAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=LayerNorm4d):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.norm3 = norm_layer(dim)
        ...
        self.attn_spatial = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,attention_mode='cosine')
...
class WindowedAttentionBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,window_size=(2,2,2,2), qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.,proj_drop=0.,num_patches=(1,1,32,64),roll=False,act_layer=nn.GELU, norm_layer=LayerNorm4d,bias_mode='earth',mask=True,**kwargs):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
...
class PanguWeatherBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,window_size=(2,2,2,2), qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.,proj_drop=0.,num_patches=(1,1,32,64),act_layer=nn.GELU, norm_layer=LayerNorm4d,depth=1,use_checkpoint=False,
                 inner_block=GlobalLocalAttentionBlock,bias_mode='earth',mask=True,**kwargs):
        super().__init__()
