...
class PatchEmbed3D(PatchEmbedND):
    ...
    def build_layers(self):
        return nn.Conv3d(self.in_channels,self.out_channnnels,self.patch_size,stride=self.patch_size,bias=self.bias,padding=self.padding)
   
class PatchEmbed4D(PatchEmbedND):

    def build_layers(self):
        return Conv4d(self.in_channels,self.out_channnnels,self.patch_size,stride=self.patch_size,bias=self.bias,padding=self.padding)
   
class ConvBlock4D(nn.Module):

    def __init__(self,in_channels,out_channels,depth=1,bias=True,skip=False,prediction_channel=None) -> None:
        super().__init__()
        layers = []
        self.in_channels = in_channels
        ...
        self.skip_type = skip
        self.depth = depth
        self.prediction_channel = prediction_channel if prediction_channel else out_channels
        if self.depth == 0:
            ...
            if not skip:
                ...
                self.skip = Conv4d(in_ch,self.out_channels,(1,1,1,1),stride=(1,1,1,1),bias=bias,padding=(0,0,0,0))
            for i in range(depth):
                layers.append(Conv4d(in_ch,self.out_channels,(3,3,3,3),stride=(1,1,1,1),bias=bias,padding=(1,1,1,1)))
                layers.append(LayerNorm4d(out_channels))
                layers.append(nn.GELU())
                in_ch = self.out_channels
            layers.append(
                Conv4d(in_ch,self.prediction_channel,(1,1,1,1),stride=(1,1,1,1),bias=bias,padding=(0,0,0,0))
            )
...
class PatchRecoverND(nn.Module):
    def __init__(self, patch_size,in_channels,out_channels,input_shape,padding=None,bias=True,decoder_depth=2,):
        '''
        ...
        self.head.append(nn.Linear(out_channels,self.in_channels * np.product(patch_size)))
...
class PatchRecover3D(PatchRecoverND):
    ...
    def post_process(self,x):
        ...
        x = x[:,:,self.padding[0]:self.padding[0]+self.input_shape[0],
...
class PatchRecover4D(PatchRecoverND):
    ...
    def post_process(self,x):
        ...
        x = rearrange(x,'N T L H W (C pT pL pH pW )-> N C (T pT) (L pL) (H pH) (W pW)',**axis_len)
        x = x[:,:,self.padding[0]:self.padding[0]+self.input_shape[0],
                    self.padding[1]:self.padding[1]+self.input_shape[1],
                    self.padding[2]:self.padding[2]+self.input_shape[2],
...
def compute_downsample_pad(input_shape,window_size,one_sided=False):
    ...
    for x,y in zip(input_shape,window_size):
        if (reminder:= x % y) == 0:
            ...
            pad_tgt = np.ceil( (y - reminder ) / 2. ).astype(int)
...
def build_downsample(in_channels,out_channels,input_shape,window_size=(2,2,2,2)):
    ...
    return PatchEmbed4D(patch_size=window_size,in_channels=in_channels,out_channels=out_channels,padding=padding,bias=True),dict(window_size=np.array(window_size).astype(float),padding=padding)
...
def build_upsample(in_channels,out_channels,input_shape,window_size=(2,2,2,2)):
    ...
    window_size = tuple([min(x,y) for x,y in zip(input_shape,window_size)])
    padding = compute_downsample_pad(input_shape,window_size)
    return PatchRecover4D(window_size,in_channels=in_channels,out_channels=out_channels,input_shape=input_shape,padding=padding,bias=True,decoder_depth=0)

def crop_4d(x,input_shape,padding=(0,0,0,0)):
    '''
    x: N C T L H W
    '''
    return x[:,:,padding[0]:padding[0]+input_shape[0],
