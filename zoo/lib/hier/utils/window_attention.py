...
def window_partition4d(x, window_size: Tuple[int, int]):
    ...
    Returns:
        windows: 
    """
    return rearrange(x,'N C (T WT) (L WL) (H WH) (W WW) -> (N T L H W )  (WT WL WH WW ) C',)
...
class WindowAttention4D(nn.Module):
    ...
    def __init__(
        ...
        self.padding = compute_downsample_pad(self.input_resolution,self.window_size,one_sided=True)
        self.actual_padding = np.concatenate([[0,v] for v in reversed(self.padding)]).astype(int).tolist()
        ...
        if self.bias_mode == 'swin':
            ...
            relative_coords = [torch.arange(-(x - 1), x, dtype=torch.float32) for x in self.window_size]
            #relative_coords_h = torch.arange(-(self.window_size[0] - 1), self.window_size[0], dtype=torch.float32)
            #relative_coords_w = torch.arange(-(self.window_size[1] - 1), self.window_size[1], dtype=torch.float32)
            relative_coords_table = torch.stack(torch.meshgrid(relative_coords)).permute(*range(1,len(self.window_size)+1), 0).contiguous().unsqueeze(0)  # 1, 2*Wh-1, 2*Ww-1, 2
            ...
            abs_coords = [torch.arange(x, dtype=torch.float32) for x in self.input_resolution] # T L H W
            ...
            abs_coords = rearrange(abs_coords,'(NT WT) (NL WL) (NH WH) (NW WW) C -> (NT NL NH NW)   (WT WL WH WW ) C',**self.window_parms)
            abs_coords_x = abs_coords[:,:,None].repeat([1,1,nL,1])
            abs_coords_y = abs_coords[:,None,].repeat([1,nL,1,1])
            if self.bias_mode == 'earth':
                ...
                abs_table = torch.stack(
                    ...
                    abs_coords_x[...,1],abs_coords_y[...,1],
                    abs_coords_x[...,2],abs_coords_y[...,2],
        ...
        if self.roll:
            ...
            shifts_iters = [
                (
                    ...
                    slice(-self.shift_size[i], None)) for i in range(len(self.window_size))
            ]
            ...
            mask_windows = rearrange(img_mask,'N  (NT WT) (NL WL) (NH WH) (NW WW) C -> (N NT NL NH NW )  (WT WL WH WW ) C',**self.window_parms)
    ...
    def transform(self,x):
        ...
        x = rearrange(x,'N C (NT WT) (NL WL) (NH WH) (NW WW) -> (N NT NL NH NW )  (WT WL WH WW ) C',**self.window_parms) # B_
        WT,WL,WH,WW = self.window_size
        NT = T // WT
        NL = L // WL
        NH = H // WH
    ...
    def inverse_transform(self,x,transform_info):
        ...
        x = rearrange(x,'(N NT NL NH NW )  (WT WL WH WW ) C -> N C (NT WT) (NL WL) (NH WH) (NW WW)',**self.window_parms,**transform_info) # B_
        if self.roll:
            x = torch.roll(x,shifts=tuple(self.shift_size),dims=(2,3,4,5))
        if self.do_padding:
    ...
    def _atten(self, x,mask=None):
        ...
        if self.bias_mode == 'swin':
            ...
            relative_position_bias = relative_position_bias_table[self.relative_position_index.view(-1)].view(
                np.product(self.window_size), np.product(self.window_size), -1)  # Wh*Ww,Wh*Ww,nH
            ...
            nL = np.product(self.window_size)
            nH = self.num_heads
            abs_position_bias = self.cpb_mlp(self.abs_table).view(-1,nL, nL, nH)  # nW * Wh*Ww, Wh*Ww * nH
            abs_position_bias = abs_position_bias.permute(0,3,1,2) # nW nH, Wh*Ww, Wh*Ww
