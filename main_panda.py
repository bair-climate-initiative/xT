...
class PANDADataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = image_handler.read_region(
            location=(0, 0), level=1, size=image_handler.level_dimensions[1]
        )
        image_handler.close()
...
dataset_show = PANDADataset(
    df_train, image_size, n_tiles, 0, transform=transforms_train
)
...
for i in range(2):
    ...
    for p in range(5):
        ...
        axarr[p].imshow(1.0 - img.transpose(0, 1).transpose(1, 2).squeeze())
